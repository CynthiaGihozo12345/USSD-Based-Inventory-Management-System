...
class Menu {
    ...
    public function menuStockOut($textArray, $phoneNumber) {
        ...
        if ($level == 1) {
            ...
            if ($action == '1') {
                global $conn;
                $stmt = $conn->prepare("SELECT p.id, s.quantity FROM products p JOIN stock s ON p.id = s.product_id WHERE p.user_id = ? AND p.product_name = ?");
                $stmt->bind_param("is", $user['id'], $productName);
                $stmt->execute();
                $result = $stmt->get_result();
                $product = $result->fetch_assoc();
    ...
    public function menuViewProducts($textArray, $phoneNumber) {
        ...
        $stmt = $conn->prepare("SELECT id, product_name, description FROM products WHERE user_id = ?");
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $response .= $row['product_name'] . " (ID: " . $row['id'] . ")\n";
                if ($row['description']) {
    ...
    public function menuViewStockInHistory($textArray, $phoneNumber) {
        ...
        $stmt = $conn->prepare("
            SELECT p.product_name, si.quantity, si.transaction_date, si.notes 
            FROM stock_in si 
    ...
    public function menuViewStockOutHistory($textArray, $phoneNumber) {
        ...
        $stmt = $conn->prepare("
            SELECT p.product_name, so.quantity, so.transaction_date, so.notes 
            FROM stock_out so 
